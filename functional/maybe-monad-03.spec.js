...
describe('The module', () => {
  ...
  it('should satisfy the third monad law of associativity', () => {
    ...
    const associativity1 = Maybe.of(3).chain(g).chain(f)
  ...
  it('should be able to apply an applicative functor', () => {
    const funWithFunctor = Maybe.of((x) => (y) => x + y + 2)
      .ap(Maybe.of(1))
      .ap(Maybe.of(2))
  ...
  it('should be able to fork', () => {
    const forkOrSpoon = Maybe.of('spoon')
      .map((x) => null)
      .fork(
        (_) => 'forked!',
        (y) => y
  ...
  it('should fork the correct direction', () => {
    const forkRight = Maybe.of('sporks')
      .map((x) => x + ' for all!')
      .fork(
        (_) => 'there is no fork',
        (y) => y
  ...
  it('should be able to forkR (right)', () => {
    const forkOrSpoon = Maybe.of('spoon').forkR((x) => 'forked right!')
    expect(forkOrSpoon).toBe('forked right!')
  })

  it('should be able to forkL (left)', () => {
    const forkOrSpoon = Maybe.of('spoon')
      .map((x) => null)
      .forkL((_) => 'forked!')
    expect(forkOrSpoon).toBe('forked!')
  ...
  it('should be able to forkL (left) and return nothing if it goes right', () => {
    const forkOrSpoon = Maybe.of('spoon').forkL((x) => 'forked right!')
    expect(forkOrSpoon).not.toBe('forked right!')
  })

  it('should be able to forkR (right) and return nothing if it goes left', () => {
    const forkOrSpoon = Maybe.of('spoon')
      .map((x) => null)
      .forkR((_) => 'forked!')
